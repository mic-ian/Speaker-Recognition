...
def get_fusion(audio_file):
    ...
    sub.call("bash /home/fusco/webapp/ivector-xvector-master/xvector/enroll_mod_ciro.sh "+audio_file+" 1",
              shell=True, executable="/bin/bash", env=my_env, cwd=audio_dir)
    xvector = np.load(audio_dir+"/data_x/embeddings.npy")
    sub.call("rm -rf "+ audio_dir + "/data_x",shell=True, executable="/bin/bash", env=my_env, cwd=audio_dir)

    
    sub.call("bash /home/fusco/webapp/ivector-xvector-master/ivector/enroll_mod_mik.sh "+audio_file,shell=True, executable="/bin/bash", env=my_env, cwd=audio_dir)
    ...
    ####  value = xvector[0:, 400:] ERRORE LOGICO #####
    value = np.zeros((1,112)) # Correzione attuale del codice da verificare sperimentalmente
    ivector = np.append(ivector,value, axis=1)
    
    xvector = xvector.reshape((xvector.shape[0], 1, xvector.shape[1]))
    ivector = ivector.reshape((ivector.shape[0], 1, ivector.shape[1]))
 
    return xvector * 0.4 + ivector * 0.6


def extract_fusion(path):
    vector_fusion = get_fusion(path)
